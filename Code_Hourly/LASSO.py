...
####################################################################################################################
# Train the LASSO model and present its errors
####################################################################################################################
...
alphas = np.logspace(-4, 2, 250)

parameters = [{"alpha": alphas}]

tuning_model=GridSearchCV(LASSO_reg,
                          param_grid=parameters,
                          scoring='neg_mean_squared_error',
                          #scoring='neg_mean_absolute_error',
                          refit=True,
                          cv=TimeSeriesSplit(n_splits=5), #5, #[(slice(None), slice(None))],
                          n_jobs=-1,
                          verbose=0)

tuning_model.fit(X_train, y_train)

print('\nLASSO regression model - best hyperparameters:')
print('V: ', tuning_model.best_params_)
print()

LASSO_reg = Lasso(**tuning_model.best_params_)
...
print(f"\nLASSO Model RMSE for training = {train_XGB_error[0]:0.5f} ")
print(f"LASSO Model nRMSE for training = {train_XGB_error[1]:0.5f} ")
print(f"LASSO Model MAE for training = {train_XGB_error[2]:0.5f} ")
print(f"LASSO Model nMAE for training = {train_XGB_error[3]:0.5f} ")
print(f"LASSO Model MAPE for training = {train_XGB_error[4]:0.5f} ")
print(f"LASSO Model MBE for training = {train_XGB_error[5]:0.5f} ")
print(f"LASSO Model Skill for training = {train_XGB_error[6]:0.5f} \n")

y_pred = LASSO_reg.predict(X_valid)

valid_LASSO_error = calculate_errors(pd.DataFrame(y_pred).iloc[:,0], y_valid, persistRMSE)
   
print(f"\nLASSO Model RMSE for validation = {valid_LASSO_error[0]:0.5f} ")
print(f"LASSO Model nRMSE for validation = {valid_LASSO_error[1]:0.5f} ")
print(f"LASSO Model MAE for validation = {valid_LASSO_error[2]:0.5f} ")
print(f"LASSO Model nMAE for validation = {valid_LASSO_error[3]:0.5f} ")
print(f"LASSO Model MAPE for validation = {valid_LASSO_error[4]:0.5f} ")
print(f"LASSO Model MBE for validation = {valid_LASSO_error[5]:0.5f} ")
print(f"LASSO Model Skill for validation = {valid_LASSO_error[6]:0.5f} \n")
...
if plotLASSO:
    ...
    ax.axhline(y = 500, color = 'k', linestyle = 'dashed', linewidth = 4)
    ax.axvline(x = 400, color = 'k', linestyle = ':', linewidth = 3)
    ax.axvline(x = 500, color = 'k', linestyle = 'dashed', linewidth = 4)
    ...
    plt.title('LASSO Regression - Validation Dataset - ' + str(STUDY_STATION) , size=20)
...
print(f"\nTrained the LASSO model in {TIC - TOC:0.4f} seconds \n"); del TIC, TOC
