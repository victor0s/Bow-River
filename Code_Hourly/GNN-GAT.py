...
class GAT(torch.nn.Module):
    def __init__(self, num_feat, num_nodes, device='cpu'):
        ...
        self.in_head = 2
        self.out_head = 1
        self.conv1 = GATConv(num_feat, self.hid, heads=self.in_head)
        self.conv2 = GATConv(self.hid*self.in_head, self.hid, heads=self.out_head)
        self.conv3 = GATConv(self.hid*self.out_head, self.hid, heads=self.out_head)
        self.conv4 = GATConv(self.hid*self.out_head, self.hid, heads=self.out_head)
...
model = GAT(data_list[0].x.shape[1], data_list[0].x.shape[0]).to(DEVICE)
...
print(f"\nCreated and sent the GAT model to {torch.cuda.get_device_name(DEVICE).upper()} in {TIC - TOC:0.4f} seconds \n")
time.sleep(7)
...
def evaluate_model(valid_dl, model, DEVICE='cpu', persistRMSE=1.0, plot=False, evalEPA=False):
    ...
    with torch.no_grad():
        ...
        if plot:
            ...
            plt.title('GAT-GNN Regression - Validation Dataset - ' + str(STUDY_STATION), size=20)
...
# Evaluate the model
####################################################################################################################
valid_GAT_errors = evaluate_model(valid_loader, model, DEVICE=DEVICE,
                                   persistRMSE=valid_persist_errors[0], plot=plotGAT, evalEPA=False)

valid_GAT_errors = list(valid_GAT_errors)
for i in range(len(valid_GAT_errors)):
    valid_GAT_errors[i] = valid_GAT_errors[i].item()

print('\nFinal results for the validation set:')

print(f"\nGAT Validation RMSE = {valid_GAT_errors[1]:.5f}")
print(f"GAT Validation nRMSE = {valid_GAT_errors[2]:.5f}")
print(f"GAT Validation MAE = {valid_GAT_errors[3]:.5f}")
print(f"GAT Validation nMAE = {valid_GAT_errors[4]:.5f}")
print(f"GAT Validation MAPE = {valid_GAT_errors[5]:.5f}")
print(f"GAT Validation MBE = {valid_GAT_errors[6]:.5f}")
print(f"GAT Validation Skill = {valid_GAT_errors[7]:.5f}")
print(f"GAT Validation R2 = {valid_GAT_errors[8]:.5f}")
print()
