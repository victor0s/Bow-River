...
class SAGE(torch.nn.Module):
    def __init__(self, num_feat, num_nodes, device='cpu'):
        ...
        self.aggreg = 'mean'
        #self.aggreg = 'sum'
        self.conv1 = SAGEConv(num_feat, self.hid, aggr=self.aggreg)
        self.conv2 = SAGEConv(self.hid, self.hid, aggr=self.aggreg)
        self.conv3 = SAGEConv(self.hid, self.hid, aggr=self.aggreg)
        self.conv4 = SAGEConv(self.hid, self.hid, aggr=self.aggreg)
...
model = SAGE(data_list[0].x.shape[1], data_list[0].x.shape[0]).to(DEVICE)
...
print(f"\nCreated and sent the SAGE model to {torch.cuda.get_device_name(DEVICE).upper()} in {TIC - TOC:0.4f} seconds \n")
time.sleep(7)
...
def evaluate_model(valid_dl, model, DEVICE='cpu', persistRMSE=1.0, plot=False, evalEPA=False):
    ...
    with torch.no_grad():
        ...
        if plot:
            ...
            plt.title('SAGE-GNN Regression - Validation Dataset - ' + str(STUDY_STATION), size=20)
            ...
            # overall - Stage
            plt.figure(figsize=(5.5*5,2.5*5))
            plt.plot(df_ML_valid.reset_index(drop=True).iloc[:, 0],
                     0.2045*(Vactuals.reshape(-1).cpu())**0.399,
                     label='Measured Water Level (m)', color='tab:green', linewidth=2.5)
            plt.fill_between(df_ML_valid.reset_index(drop=True).iloc[:, 0],
                             0.2045*(Vactuals.reshape(-1).cpu())**0.399,
                             color='tab:green', alpha=0.4)
            plt.plot(df_ML_valid.reset_index(drop=True).iloc[:, 0],
                     0.2045*(Vpredictions.reshape(-1).cpu())**0.399,
                     linestyle = 'dashdot', linewidth=2.5, label='Forecasted Water Level (m)', color='midnightblue')
            plt.legend(shadow=True, fontsize=14)
            plt.gcf().autofmt_xdate()
            plt.title('SAGE-GNN Regression - Validation Dataset - ' + str(STUDY_STATION), size=20)
            plt.xlabel('Date', size=14)
...
# Evaluate the model
####################################################################################################################
valid_SAGE_errors = evaluate_model(valid_loader, model, DEVICE=DEVICE,
                                   persistRMSE=valid_persist_errors[0], plot=plotSAGE, evalEPA=False)

valid_SAGE_errors = list(valid_SAGE_errors)
for i in range(len(valid_SAGE_errors)):
    valid_SAGE_errors[i] = valid_SAGE_errors[i].item()

print('\nFinal results for the validation set:')

print(f"\nSAGE Validation RMSE = {valid_SAGE_errors[1]:.5f}")
print(f"SAGE Validation nRMSE = {valid_SAGE_errors[2]:.5f}")
print(f"SAGE Validation MAE = {valid_SAGE_errors[3]:.5f}")
print(f"SAGE Validation nMAE = {valid_SAGE_errors[4]:.5f}")
print(f"SAGE Validation MAPE = {valid_SAGE_errors[5]:.5f}")
print(f"SAGE Validation MBE = {valid_SAGE_errors[6]:.5f}")
print(f"SAGE Validation Skill = {valid_SAGE_errors[7]:.5f}")
print(f"SAGE Validation R2 = {valid_SAGE_errors[8]:.5f}")
print()
