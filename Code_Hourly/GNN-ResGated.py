...
class ResGatedGraph(torch.nn.Module):
    def __init__(self, num_feat, num_nodes, device='cpu'):
        ...
        self.hid = 128
        self.conv1 = ResGatedGraphConv(num_feat, self.hid)
        self.conv2 = ResGatedGraphConv(self.hid, self.hid)
        self.conv3 = ResGatedGraphConv(self.hid, self.hid)
        self.conv4 = ResGatedGraphConv(self.hid, self.hid)
...
model = ResGatedGraph(data_list[0].x.shape[1], data_list[0].x.shape[0]).to(DEVICE)
...
print(f"\nCreated and sent the ResGated model to {torch.cuda.get_device_name(DEVICE).upper()} in {TIC - TOC:0.4f} seconds \n")
time.sleep(7)
...
def evaluate_model(valid_dl, model, DEVICE='cpu', persistRMSE=1.0, plot=False, evalEPA=False):
    ...
    with torch.no_grad():
        ...
        if plot:
            ...
            plt.title('ResGated-GNN Regression - Validation Dataset - ' + str(STUDY_STATION), size=20)
...
# Evaluate the model
####################################################################################################################
valid_ResGated_errors = evaluate_model(valid_loader, model, DEVICE=DEVICE,
                                   persistRMSE=valid_persist_errors[0], plot=plotResGated, evalEPA=False)

valid_ResGated_errors = list(valid_ResGated_errors)
for i in range(len(valid_ResGated_errors)):
    valid_ResGated_errors[i] = valid_ResGated_errors[i].item()

print('\nFinal results for the validation set:')

print(f"\nResGated Validation RMSE = {valid_ResGated_errors[1]:.5f}")
print(f"ResGated Validation nRMSE = {valid_ResGated_errors[2]:.5f}")
print(f"ResGated Validation MAE = {valid_ResGated_errors[3]:.5f}")
print(f"ResGated Validation nMAE = {valid_ResGated_errors[4]:.5f}")
print(f"ResGated Validation MAPE = {valid_ResGated_errors[5]:.5f}")
print(f"ResGated Validation MBE = {valid_ResGated_errors[6]:.5f}")
print(f"ResGated Validation Skill = {valid_ResGated_errors[7]:.5f}")
print(f"ResGated Validation R2 = {valid_ResGated_errors[8]:.5f}")
print()
