...
class GCN(torch.nn.Module):
    def __init__(self, num_feat, num_nodes, device='cpu'):
        ...
        self.in_head = 1
        self.out_head = 1
        self.conv1 = GCNConv(num_feat, self.hid, heads=self.in_head)
        self.conv2 = GCNConv(self.hid*self.in_head, self.hid, heads=self.out_head)
        self.conv3 = GCNConv(self.hid*self.out_head, self.hid, heads=self.out_head)
        self.conv4 = GCNConv(self.hid*self.out_head, self.hid, heads=self.out_head)
...
model = GCN(data_list[0].x.shape[1], data_list[0].x.shape[0]).to(DEVICE)
...
print(f"\nCreated and sent the GCN model to {torch.cuda.get_device_name(DEVICE).upper()} in {TIC - TOC:0.4f} seconds \n")
time.sleep(7)
...
def evaluate_model(valid_dl, model, DEVICE='cpu', persistRMSE=1.0, plot=False, evalEPA=False):
    ...
    with torch.no_grad():
        ...
        if plot:
            ...
            plt.title('GCN-GNN Regression - Validation Dataset - ' + str(STUDY_STATION), size=20)
...
# Evaluate the model
####################################################################################################################
valid_GCN_errors = evaluate_model(valid_loader, model, DEVICE=DEVICE,
                                   persistRMSE=valid_persist_errors[0], plot=plotGCN, evalEPA=False)

valid_GCN_errors = list(valid_GCN_errors)
for i in range(len(valid_GCN_errors)):
    valid_GCN_errors[i] = valid_GCN_errors[i].item()

print('\nFinal results for the validation set:')

print(f"\nGCN Validation RMSE = {valid_GCN_errors[1]:.5f}")
print(f"GCN Validation nRMSE = {valid_GCN_errors[2]:.5f}")
print(f"GCN Validation MAE = {valid_GCN_errors[3]:.5f}")
print(f"GCN Validation nMAE = {valid_GCN_errors[4]:.5f}")
print(f"GCN Validation MAPE = {valid_GCN_errors[5]:.5f}")
print(f"GCN Validation MBE = {valid_GCN_errors[6]:.5f}")
print(f"GCN Validation Skill = {valid_GCN_errors[7]:.5f}")
print(f"GCN Validation R2 = {valid_GCN_errors[8]:.5f}")
print()
